...
D = np.concatenate((B,C,A), axis=1)
im4 = Image.fromarray((D * 255).astype(np.uint8))
im4.save("4.jpeg")

E = cv2.addWeighted(B, 1, A, 0.5, 0)
im5 = Image.fromarray((E * 255).astype(np.uint8))
im5.save("overlay.jpeg")

# np.save('uncertainty_maps.npy', saliency_maps)